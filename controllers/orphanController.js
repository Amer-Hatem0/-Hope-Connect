// ✅ controllers/orphanController.js (مع logging + email)
...
exports.updateOrphan = (req, res) => {
  ...
  db.query('UPDATE orphans SET ? WHERE id = ?', [updatedData, id], (err, result) => {
    if (err) {
      logger.error(`Error updating orphan ID ${id}:`, err);
      return res.status(500).json({ error: err });
    }
    if (result.affectedRows === 0) {
      logger.warn(`Attempt to update non-existent orphan ID: ${id}`);
      return res.status(404).json({ message: 'The orphan is not present' });
    }
    logger.info(`Orphan updated - ID: ${id}`);
...
exports.deleteOrphan = (req, res) => {
  ...
  db.query('DELETE FROM orphans WHERE id = ?', [id], (err, result) => {
    if (err) {
      logger.error(`Error deleting orphan ID ${id}:`, err);
      return res.status(500).json({ error: err });
    }
    if (result.affectedRows === 0) {
      logger.warn(`Attempt to delete non-existent orphan ID: ${id}`);
      return res.status(404).json({ message: 'The orphan is not present' });
    }
    logger.info(`Orphan deleted - ID: ${id}`);
